    ...
    for activity in ["all"]:
        ...
        for i in range(200):
            for batch_idx, (train_x, train_y) in enumerate(train_loader):
                ...
                recons_loss = F.mse_loss(train_xr, train_x)
                kld_loss = torch.mean(-0.1 * torch.sum(1 + log_var - mu ** 2 - log_var.exp(), dim = 1), dim = 0)

                loss = torch.mean(recons_loss + kld_loss + vaeLoss)
                loss.backward()
        ...
        torch.save(encodermodel.state_dict(), '/home/omid/pycharm/Mobi/models/obs_motion_encoder_'+activity+str(z_dim))
        torch.save(decodermodel.state_dict(), '/home/omid/pycharm/Mobi/models/obs_motion_decoder_'+activity+str(z_dim))
...
encodermodel_0.load_state_dict(torch.load('/home/omid/pycharm/Mobi/models/obs_motion_encoder_all'+str(z_dim)))
if usecuda:
    encodermodel_0.cuda(idgpu)
decodermodel_0 = Decoder().double()
decodermodel_0.load_state_dict(torch.load('/home/omid/pycharm/Mobi/models/obs_motion_decoder_all'+str(z_dim)))
if usecuda:
