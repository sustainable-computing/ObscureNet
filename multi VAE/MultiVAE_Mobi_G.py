...
for z_dim in zed:
    ...
    for activity in ["all"]:
        ...
        torch.save(encodermodel.state_dict(), '/home/omid/pycharm/Mobi/models/multi_vae_encoder_'+str(activity)+str(z_dim)+'gender')
        torch.save(decodermodel.state_dict(), '/home/omid/pycharm/Mobi/models/multi_vae_decoder_'+str(activity)+str(z_dim)+'gender')
...
for activity in [0]:
    ...
    encodermodel.load_state_dict(torch.load('/home/omid/pycharm/Mobi/models/multi_vae_encoder_'+str(activity)+str(z_dim)+'gender'))
    if usecuda:
        encodermodel.cuda(idgpu)

    decodermodel = Decoder().double()
    decodermodel.load_state_dict(torch.load('/home/omid/pycharm/Mobi/models/multi_vae_decoder_'+str(activity)+str(z_dim)+'gender'))
    if usecuda:
        decodermodel.cuda(idgpu)
