...
w_adversarial = 0.5 # 0.1 0.2 0.5 1 2 10
# beta = 0.2 # 0.1 0.2 1 2 10

# beta = 0.5

accuracy_inference = np.zeros((6, 2))
# activity_inference = np.zeros(5)

for loooop, beta in enumerate([0.1, 0.2, 0.5, 1, 2, 10]):
    for z_dim in zed:
        ...
        for activity in range(4):#[AI]:
            ...
            for i in range(200):
                for batch_idx, (train_x, train_y) in enumerate(train_loader):
                    ...
                    kld_loss = torch.mean(-beta * torch.sum(1 + log_var - mu ** 2 - log_var.exp(), dim = 1), dim = 0)

                    loss = (recons_loss + kld_loss)/150 + w_adversarial*vaeLoss
                    loss.backward()
            ...
            torch.save(encodermodel.state_dict(), '/home/omid/pycharm/Mobi/models/motion_to_encoder_'+str(activity)+str(z_dim)+str(w_adversarial)+"_beta_"+str(beta))
            torch.save(decodermodel.state_dict(), '/home/omid/pycharm/Mobi/models/motion_to_decoder_'+str(activity)+str(z_dim)+str(w_adversarial)+"_beta_"+str(beta))
    ...
    encodermodel_0.load_state_dict(torch.load('/home/omid/pycharm/Mobi/models/motion_to_encoder_0'+str(z_dim)+str(w_adversarial)+"_beta_"+str(beta)))
    if usecuda:
        encodermodel_0.cuda(idgpu)
    decodermodel_0 = Decoder().double()
    decodermodel_0.load_state_dict(torch.load('/home/omid/pycharm/Mobi/models/motion_to_decoder_0'+str(z_dim)+str(w_adversarial)+"_beta_"+str(beta)))
    if usecuda:
        decodermodel_0.cuda(idgpu)

    encodermodel_1 = Encoder().double()
    encodermodel_1.load_state_dict(torch.load('/home/omid/pycharm/Mobi/models/motion_to_encoder_1'+str(z_dim)+str(w_adversarial)+"_beta_"+str(beta)))
    if usecuda:
        encodermodel_1.cuda(idgpu)
    decodermodel_1 = Decoder().double()
    decodermodel_1.load_state_dict(torch.load('/home/omid/pycharm/Mobi/models/motion_to_decoder_1'+str(z_dim)+str(w_adversarial)+"_beta_"+str(beta)))
    if usecuda:
        decodermodel_1.cuda(idgpu)

    encodermodel_2 = Encoder().double()
    encodermodel_2.load_state_dict(torch.load('/home/omid/pycharm/Mobi/models/motion_to_encoder_2'+str(z_dim)+str(w_adversarial)+"_beta_"+str(beta)))
    if usecuda:
        encodermodel_2.cuda(idgpu)
    decodermodel_2 = Decoder().double()
    decodermodel_2.load_state_dict(torch.load('/home/omid/pycharm/Mobi/models/motion_to_decoder_2'+str(z_dim)+str(w_adversarial)+"_beta_"+str(beta)))
    if usecuda:
        decodermodel_2.cuda(idgpu)

    encodermodel_3 = Encoder().double()
    encodermodel_3.load_state_dict(torch.load('/home/omid/pycharm/Mobi/models/motion_to_encoder_3'+str(z_dim)+str(w_adversarial)+"_beta_"+str(beta)))
    if usecuda:
        encodermodel_3.cuda(idgpu)
    decodermodel_3 = Decoder().double()
    decodermodel_3.load_state_dict(torch.load('/home/omid/pycharm/Mobi/models/motion_to_decoder_3'+str(z_dim)+str(w_adversarial)+"_beta_"+str(beta)))
    if usecuda:
    ...
    accuracy_inference[loooop, 0] = act_gen
    accuracy_inference[loooop, 1] = act_acc

# numpy_data = np.array([[1, 2], [3, 4]])
df = pd.DataFrame(data=accuracy_inference, index=["row1", "row2", "row3", "row4", "row5", "row6"], columns=["gen", "act"])

df.to_csv('data_alpha_'+str(w_adversarial)+'.csv', index=False)